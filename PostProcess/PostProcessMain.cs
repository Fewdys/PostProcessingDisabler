...
{
    public class PostProcessing : MelonMod
    {
            ...
            for (int i = 0; i < methodInfos.Length; i++)
            {
                ...
                for (int j = 0; j < mt.Length; j++)
                {
                    ...
                    if (mt[j].ReadAsObject().ToString().Contains("OnPlayerJoin"))
                    {
                        ...
                        MelonUtils.NativeHookAttach((IntPtr)(&methodPointer), typeof(PostProcessing).GetMethod(nameof(OnJoin), BindingFlags.Static | BindingFlags.NonPublic)!.MethodHandle.GetFunctionPointer());
